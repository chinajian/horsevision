...
<nav class="navbar navbar-default child-nav">
    <h5 class="nav pull-left">修改奖品</h5>
...
<form class="form-horizontal" id="modForm">
    <input type="hidden" class="form-control input-sm" name="Prize[pid]" id="pid" value="<?php echo $prize['pid']?>">
    <div class="form-group">
        <label for="prize_name" class="col-sm-2 control-label">奖品名称<span class="mandatory">*</span></label>
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Prize[prize_name]" id="prize_name" placeholder="奖品名称" value="<?php echo $prize['prize_name']?>">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9">
            <ul id="upload">
                <?php 
                    $i = 0;
                    foreach($prize['prize_img'] as $k => $v){
                ?>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-9 iCheck">
            <label class="radio-inline">
                <input type="radio" name="Prize[is_red_packet]" value="0" <?php if($prize['is_red_packet'] === '0'){?>checked<?php }?>> 否
            </label>
            <label class="radio-inline">
                <input type="radio" name="Prize[is_red_packet]" value="1" <?php if($prize['is_red_packet'] === '1'){?>checked<?php }?>> 是
            </label>
        </div>
    </div>
    <div class="form-group" <?php if($prize['is_red_packet'] === '0'){?>style="display: none"<?php }?>>
        <label for="red_packet_money" class="col-sm-2 control-label">红包金额</label>
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Prize[red_packet_money]" id="red_packet_money" placeholder="红包金额" value="<?php echo $prize['red_packet_money']?>">
...
<script type="text/javascript">
    ...
    $("#mod").click(function(){
        ...
        /*相册图片，循环出一个input列表<<<*/
        jajax("<?php echo Url::to(['prize/mod-prize'])?>", $('#modForm').serialize());
    })
