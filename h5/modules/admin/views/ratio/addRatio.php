...
<nav class="navbar navbar-default child-nav">
    <h5 class="nav pull-left">添加配比</h5>
...
<form class="form-horizontal" id="addForm">
        ...
        <div class="col-sm-9">
            <select class="form-control" name="Ratio[sid]" id="sid">
                ...
                <?php foreach($season as $k => $v){?>
                    <option value="<?php echo $v['sid'];?>"><?php echo $v['season_name'];?>（<?php echo date('Y-m-d H:i:s', $v['luckydraw_begin_time']);?> ~ <?php echo date('Y-m-d H:i:s', $v['luckydraw_end_time']);?>）</option>
                <?php }?>
            </select>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Ratio[total_num]" id="total_num" placeholder="奖品总数量" value="">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9">
            <?php foreach($prize as $k => $v){?>
                <div class="choosePrize">
                    <label>
                        <input type="radio" name="Ratio[pid]" value="<?php echo $v['pid'];?>"><?php echo $v['prize_name'];?>
                    </label>
                </div>
            <?php }?>
        </div>
...
<script type="text/javascript">
    /*添加*/
    $("#add").click(function(){
        jajax("<?php echo Url::to(['ratio/add-ratio'])?>", $('#addForm').serialize());
    })
